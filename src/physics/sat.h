...
static float2 sat_float2_combine(float2 a, float2 b) {
  if (b.x < a.x)
    a.x = b.x;
  if (b.y > a.y)
    a.y = b.y;
...
static bool sat_intersect(vec2 *vs1, sz n1, vec2 *vs2, sz n2, float2 *minimal) {
  ...
  // TODO: we assume that having the same axes is basically impossible
  norm2 *axes = (norm2 *)calloc(sizeof(vec2), n1 + n2);
  ...
  for (sz i = 0; i < n1 + n2; i++) {
    ...
    is_overlap |= sat_float2_overlap(a, b);

    if (minimal) {
      float2 overlap = sat_float2_combine(a, b);
